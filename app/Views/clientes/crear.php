...
<h1 style="text-align:center">Añadir Nuevo Cliente</h1>
<?php if (!empty($error)): ?>
...
<form id="formCliente" action="<?php echo APP_URL; ?>/apicliente/crear" method="POST" autocomplete="off">
    <div class="form-group"><label for="dni">DNI:</label><input type="text" id="dni" name="dni" required></div>
    ...
    <div class="form-group"><label for="telefono">Teléfono:</label><input type="text" id="telefono" name="telefono" required></div>
    ...
    <div style="display: flex; justify-content: center; margin-top: 20px;">
        <button type="submit" style="min-width: 200px;">Guardar Cliente</button>
    </div>
</form>
</div>
<script>
...
form.addEventListener('submit', function(e) {
    ...
    .then(res => {
        if (res.body.error) {
            ...
            window.location = '<?php echo APP_URL; ?>/cliente?success=1';
        } else {
...
button { background-color:#007bff; color:white; padding:13px 0; border:none; border-radius:6px; font-size:17px; font-weight: bold; cursor:pointer; width:245px; transition:all .12s }
