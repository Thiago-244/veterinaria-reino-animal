...
<h1 style="text-align:center">Añadir Nueva Raza</h1>
<?php if (!empty($error)): ?>
...
<form id="formRaza" action="<?php echo APP_URL; ?>/apiraza/crear" method="POST" autocomplete="off">
        ...
        <select id="id_especie" name="id_especie" required>
            ...
            <?php foreach ($especies as $especie): ?>
                <option value="<?php echo (int)$especie['id']; ?>" 
                        <?php echo (isset($raza['id_especie']) && $raza['id_especie'] == $especie['id']) ? 'selected' : ''; ?>>
    ...
    <div class="form-group">
        ...
        <input type="text" id="nombre" name="nombre" required maxlength="50" 
               ...
               value="<?php echo isset($raza['nombre']) ? htmlspecialchars($raza['nombre']) : ''; ?>">
    ...
    <div style="display: flex; justify-content: center; gap: 20px; margin-top: 20px;">
        <button type="submit" style="min-width: 245px;">💾 Guardar Raza</button>
...
form.addEventListener('submit', function(e) {
    ...
    .then(res => {
        if (res.body.error) {
            ...
            setTimeout(() => {
                window.location = '<?php echo APP_URL; ?>/raza?success=1';
            }, 1500);
        } else {
